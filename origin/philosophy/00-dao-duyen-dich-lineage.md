...
Các tài liệu v2.3.6/v2.3.7 đã lượng hóa hướng này và đặt nguyên tắc rằng quẻ không quyết định sự việc; quẻ là một snapshot của trạng thái tại sát-na quan sát.

## 2. Vô thường và vô ngã trong kiến trúc

Mô hình không nên gán bản chất cố định cho node. Trạng thái chỉ có nghĩa trong điều kiện, vị trí và thời điểm cụ thể. v2.8.7+ Rev.A giữ quẻ như snapshot tại t0 và cấm dùng quan sát mới để sửa ngược trạng thái đã khóa.

Đây là nền để chuyển từ các nhãn bản thể mạnh sang ba phương thức biểu đạt:
...
Trong lineage cũ, quẻ là bộ mã hóa snapshot/trạng thái chứ không phải tác nhân nhân quả tuyệt đối. v2.3.6 xác định quẻ chỉ là snapshot của Dòng và dự báo nhằm đánh giá xu hướng chuyển hóa, không xác định kết quả tất định.

## 4. Truy vết và điều kiện

Một kết luận phải truy vết được về dữ liệu và các toán tử đã dùng. UK1–UK8 của v2.3.6 đặt các yêu cầu về semantic independence, primitive completeness, observer equivalence, information preservation, bidirectional traceability, fusion neutrality, decoder isolation và extensibility.

## 5. Kết quả kiến trúc hiện tại

Không xem `Entity`, `Actor`, `Object`, `Duyên` hay bất kỳ nhãn nào là bản chất cố định. Chúng chỉ là định danh/khung quy chiếu phục vụ quan sát.
